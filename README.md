...
This is a Vulkan layer that forces a specific physical device to be used. This is useful for Vulkan games which do not provide an option to choose the device themselves. It was originally written in C++ and has since been rewritten in Rust.

Compiling requires a Rust toolchain and the GTK 3 development files.
...
`VULKAN_DEVICE_INDEX` also accepts `name:<substring>` to match a device by name instead of index, `letmechoose` to pick the device from a popup window at launch, and `letmechoose:<family>` to do the same but remember the choice for that family afterward.
